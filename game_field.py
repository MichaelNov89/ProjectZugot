...
def gen_mines():
    for i in range(20):
        ...
        while ((x<consts.SOLDIER_COLS and y<consts.SOLDIER_ROWS) or (x>consts.flag_col and y>consts.flag_row)) and ((mine_field[y][x]==consts.CELL_EMPTY) or (mine_field[y][x+1]==consts.CELL_EMPTY) or (mine_field[y][x+2]==consts.CELL_EMPTY)):
            x = random.randint(0, consts.BOARD_COLS - 4)
            y = random.randint(0, consts.BOARD_ROWS - 4)
