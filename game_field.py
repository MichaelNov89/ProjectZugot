...
place_grass_list = []
mine_field = [[consts.CELL_EMPTY for i in range(consts.BOARD_COLS)] for j in
...
def random_place_grass():
    for i in range(20):
        ...
        while (
                x <= consts.CELL_SIZE * consts.SOLDIER_COLS and y <= consts.CELL_SIZE * consts.SOLDIER_ROWS) or (
                x >= consts.CELL_SIZE * consts.flag_col - 40 and y >= consts.CELL_SIZE * consts.flag_row - 40):
            x = random.randint(0, consts.WINDOW_WIDTH - 40)
            y = random.randint(0, consts.WINDOW_HEIGHT - 40)
...
def check_can_place_mines(mine_pos):
    ...
    if x < consts.SOLDIER_COLS and y < consts.SOLDIER_ROWS:
        return False
    if mine_field[y][x] != consts.CELL_EMPTY or mine_field[y][
        x + 1] != consts.CELL_EMPTY or mine_field[y][
        x + 2] != consts.CELL_EMPTY:
        return False

    return True


def set_grass_list(grass_list_to_set):
    place_grass_list = grass_list_to_set


def set_mine_field(mine_field_to_set):
    mine_field_matrix = mine_field_to_set


def set_mines_positions(mines_positions_to_set):
    mines_positions = mines_positions_to_set
